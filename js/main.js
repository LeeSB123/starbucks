...
// Badge img가 스크롤을 내리면(스크롤 값이 높아지면) 사라지도록 + 스크롤업 버튼 보이기&숨기기
...
}, 300)); // 300=0.3초 함수가 한 번에 실행되는 것을 0.3초 단위로 부하를 주어서 한번에 다량 실행 방지
// _.throttle(함수,시간) <- 시간은 ms 단위로
...
// 스타벅스 프로모션을 클릭하면 아래의 프로모션 이미지 목록이 숨김 & 보이는 기능
...
spyEls.forEach(function (spyEl) {
  // Scene 메소드는 scrollMagic 라이브러리로 특정한 요소를 감시하는 옵션 지정, setClassToggle 클래스를 넣었다 뺏다 하기를 지정, addTo는 컨트롤러
  // 특정 섹션이 화면에 보이기 시작하면 애니메이션을 추가해주는 것
  new ScrollMagic
    .Scene({
      triggerElement: spyEl, // 보여짐 여부를 감시할 요소 지정
      triggerHook: .8 // 화면(view port)의 0.8 지점을 넘어갈 때, 감지 (맨위 = 0 , 맨밑 = 1) + main.css에 있는 애니메이션 날아오는 효과도 이 지점에서 실행
    })
    .setClassToggle(spyEl, 'show') // 토글할 클래스 이름이 show
    .addTo(new ScrollMagic.Controller());
});

// footer copyright에서 연도 계산하여 출력
const thisYear = document.querySelector('.this-year');
thisYear.textContent = new Date().getFullYear(); // 2021의 올해의 해가 나옴